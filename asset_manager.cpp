...
std::map<string, Frame> parseFrames(XmlParser &parser, XmlNode parentNode)
{
  ...
  for (auto &&node : childNodes) {
    if (parser.nodeNameCompare(node, "row")) {
      ...
      try {
        horizontalPadding =
            std::stoi(parser.getAttribute(node, "horizontalPadding"));
        verticalPadding =
            std::stoi(parser.getAttribute(node, "verticalPadding"));
      } catch (...) {
...
AssetManager::AssetManager(VideoManager &videoManager, SoundPlayer &soundPlayer,
...
void AssetManager::loadImage(int id, string path, int frameWidth,
                             int frameHeight)
{
...
void AssetManager::draw(Uint32 windowID, int id, Rectangle _destRect, int row,
                        int frameNum)
{
  Image *image = this->getImage(id);
...
void AssetManager::stopSound(int id) { mSoundPlayer.deleteSound(id); }
