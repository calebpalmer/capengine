                           ...
                           CapEngine::Colour colour)
{
  ...
  if (surface->format->BytesPerPixel == 2) {
    pixel = malloc(2);
    *(Uint16 *)pixel =
        createUint16Pixel(surface->format, colour.m_r, colour.m_g, colour.m_g);
    ((Uint16 *)surface->pixels)[offset / 2] = *(Uint16 *)pixel;
  } else if (surface->format->BytesPerPixel == 3) {
    pixel = malloc(3);
    *(Uint32 *)pixel = createUint32Pixel(surface->format, colour.m_r,
                                         colour.m_g, colour.m_b, colour.m_a);
                               ...
                               Pixel solidPixel, int numNeighbours, bool above)
{
  ...
  if (above) {
    ...
    for (int i = x - 1; i >= x - numNeighbours; i--) {
      ...
      if (pixelsEqual(pixel, solidPixel)) {
        while (j > 0 &&
               pixelsEqual(getPixelComponents(surface, i, j--), solidPixel)) {
        }
        points.push_back(Vector(i, j));
      }
      //   if the pixel is not solid, look down each pixel until the first solid
      //   pixel is found, unless the pixel to the right is not solid
      else {
