...
template <class ObjectManager> class Scene2d final
{
public:
...
private:
  ...
  ObjectManager
      m_objectManager; //<! Holds the objects and performs collision checking.
...
Scene2d<ObjectManager>::Scene2d(const jsoncons::json &in_json) : m_camera(0, 0)
{
  this->load(in_json);
}
...
/**
 \param in_ms
   The timestep.
*/
template <class ObjectManager> void Scene2d<ObjectManager>::update(double in_ms)
{
  // update layers
  for (auto &&i : m_layers) {
...
void Scene2d<ObjectManager>::render(uint32_t in_windowId)
{
  ...
  // render objects
  for (auto &&pObject :
       m_objectManager.getObjects(m_camera.getViewingRectangle())) {
