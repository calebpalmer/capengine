 ...
 */
WindowWidget::WindowWidget(const std::string &name, int width, int height,
                           bool resizable)
                                                   ...
                                                   bool resizable)
{
  std::shared_ptr<WindowWidget> pWindowWidget(
      new WindowWidget(name, width, height, resizable));
...
void WindowWidget::show()
{
  ...
  WindowParams windowParams = {m_windowName, m_width,     m_height,
                               32,           // bpp,
                               m_fullScreen, // fullscreen,
                               false,        // opengl
                               m_resizable,  // resizable
                               m_maximized,  m_windowName};
...
/**
 * \param eventSubscriber - The EventSubscriber receiving the signals to
 * subscribe to
 */
void WindowWidget::registerSignals(EventSubscriber &eventSubscriber)
{
  m_handleMouseMotionConnection =
      eventSubscriber.m_mouseMotionEventSignal.connect(std::bind(
          &WindowWidget::handleMouseMotionEvent, this, std::placeholders::_1));

  m_handleMouseButtonConnection =
      eventSubscriber.m_mouseButtonEventSignal.connect(std::bind(
          &WindowWidget::handleMouseButtonEvent, this, std::placeholders::_1));

  m_handleMouseWheelConnection =
      eventSubscriber.m_mouseWheelEventSignal.connect(std::bind(
          &WindowWidget::handleMouseWheelEvent, this, std::placeholders::_1));

  m_handleKeyboardConnection =
      eventSubscriber.m_keyboardEventSignal.connect(std::bind(
          &WindowWidget::handleKeyboardEvent, this, std::placeholders::_1));

  m_handleWindowConnection = eventSubscriber.m_windowEventSignal.connect(
      std::bind(&WindowWidget::handleWindowEvent, this, std::placeholders::_1));

  m_handleTextInputConnection =
      eventSubscriber.m_textInputEventSignal.connect(std::bind(
          &WindowWidget::handleTextInputEvent, this, std::placeholders::_1));
}
...
*/
boost::signals2::scoped_connection WindowWidget::registerWindowClosedSignal(
    std::function<void(WindowWidget *)> slot)
{
...
//! Widget::getPosition
SDL_Rect WindowWidget::getPosition() const { return {0, 0, m_width, m_height}; }
