...
Time::Time(const Time& copiedTime){
  time = copiedTime.time;
}
...
double Time::subtractTime(const Time* timeToSubtract){
  ...
  return ((time.tv_sec * 1000) + (time.tv_usec / 1000)) - ((timeToSubtract->time.tv_sec * 1000) + (timeToSubtract->time.tv_usec / 1000));
}
