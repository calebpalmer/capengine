...
TextButton::TextButton(Uint32 windowID, std::string text, std::string font,
    ...
    : m_windowID(windowID), m_text(text), m_font(font), m_fontSize(fontSize),
      m_selected(false), m_position(position), m_callback(nullptr),
      m_fontColour(inactiveColour), m_pSelectedTexture(nullptr)
{
  // get surface
  FontManager fontManager;
  Surface *textSurfaceInactive =
      fontManager.getTextSurface(m_font, m_text, m_fontSize, inactiveColour.m_r,
                                 inactiveColour.m_g, inactiveColour.m_b);
...
void TextButton::render()
{
  ...
  if (m_selected) {
    ...
    Locator::videoManager->drawTexture(m_windowID, m_pSelectedTexture, nullptr,
                                       &rect);
  }
}
...
void TextButton::executeCallback() { m_callback(m_context); }
