...
/** 
 ...
 \return 
*/
std::shared_ptr<DialogState> DialogState::create(const std::string &msg, callback_t callback){
...
bool DialogState::onLoad(){
	std::shared_ptr<WindowWidget> pWindow = this->createWindow(m_kTitle, m_kWidth, m_kHeight);
	pWindow->show();

	std::shared_ptr<LinearLayout> pVerticalLayout = LinearLayout::create(LinearLayout::Orientation::Vertical);
	std::shared_ptr<Label> pLabel = Label::create(m_msg);
	pVerticalLayout->addWidget(pLabel);
	
	std::shared_ptr<LinearLayout> pHorizontalLayout = LinearLayout::create(LinearLayout::Orientation::Horizontal);
	...
	m_cancelSignalConnection = pCancelButton->addOnButtonClickedHandler([&](){
			this->m_done = true;
		});
