...
namespace CapEngine
{
  ...
  Window::Window(SDL_Window *pWindow, SDL_Renderer *pRenderer,
                 Viewport viewport, WindowParams windowParams) : m_window(pWindow), m_renderer(pRenderer), m_viewport(viewport),
                                                                 m_windowParams(windowParams) {}

  bool VideoManager::instantiated = false;

  VideoManager::VideoManager() : up_fontManager(new FontManager()), logger(nullptr), m_pWindow(nullptr), m_pRenderer(nullptr), initialized(false), m_transformationMatrix(Matrix::createIdentityMatrix())
  {
  ...
  VideoManager::VideoManager(Logger *loggerIn) : up_fontManager(new FontManager()), m_pWindow(nullptr), m_pRenderer(nullptr), logger(loggerIn), initialized(false),
...
*/
  real VideoManager::getTextureWidth(Texture *texture) const
  {
    ...
    {
      ostringstream error;
      error << "Unable to get texture width" << endl
            << SDL_GetError();
 ...
 */
  real VideoManager::getTextureHeight(Texture *texture) const
  {
    ...
    {
      ostringstream error;
      error << "Unable to get texture height" << endl
            << SDL_GetError();
      logger->log(error.str(), Logger::CERROR, __FILE__, __LINE__);
      throw CapEngineException(error.str());
    }
 ...
 */
  ...
  {
    SDL_Texture *texture = SDL_CreateTexture(m_pRenderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, width, height);
    if (texture == nullptr)
    {
      ostringstream error;
      error << "Error creating texture" << endl
            << SDL_GetError();
      logger->log(error.str(), Logger::CERROR, __FILE__, __LINE__);
      throw CapEngineException(error.str());
    }
