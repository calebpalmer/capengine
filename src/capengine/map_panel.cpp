...
void MapPanel::doLocationInit()
{
    ...
    m_translationMatrix =
        ...
        Matrix::createTranslationMatrix(
            static_cast<double>(scaledRect.x) - static_cast<double>(dstRect.x) + static_cast<double>(buffer),
            static_cast<double>(scaledRect.y) - static_cast<double>(dstRect.y) + static_cast<double>(buffer), 0.0);
...
void MapPanel::render()
{
    ...
    if (!m_locationInitialized) doLocationInit();
    ...
    if (maybeControl != boost::none) {
        ...
        if (pTileCopyControl != nullptr) {
            if (pTileCopyControl->isDragging()) {
                ...
                Rect dragRect = {maybeInitialCoords->first, maybeInitialCoords->second, x - maybeInitialCoords->first,
...
void MapPanel::handleLeftMouseButtonUp(SDL_MouseButtonEvent event)
{
    ...
    int y = 0;
    SDL_GetMouseState(&x, &y);
    if (!pointInRect({x, y}, {m_x, m_y, m_width, m_height})) return;
    ...
    if (event.type == SDL_KEYDOWN) {
        switch (event.keysym.sym) {
            case SDLK_EQUALS: {
                ...
                if (keyboardState[SDL_SCANCODE_LSHIFT] == 1 || keyboardState[SDL_SCANCODE_RSHIFT] == 1)
            ...
            case SDLK_RIGHT:
                m_translationMatrix =
                    m_translationMatrix * Matrix::createTranslationMatrix((-1.0) * kTranslationIncrement, 0.0, 0.0);
                // TODO fix this
                // m_translationMatrix.getRowVectorRef(3).setD(1.0);
                break;
            case SDLK_LEFT:
                m_translationMatrix =
                    m_translationMatrix * Matrix::createTranslationMatrix(kTranslationIncrement, 0.0, 0.0);
                // TODO fix this
                // m_translationMatrix.getRowVectorRef(3).setD(1.0);
                break;
            case SDLK_UP:
                m_translationMatrix =
                    m_translationMatrix * Matrix::createTranslationMatrix(0.0, kTranslationIncrement, 0.0);
                // TODO fix this
                // m_translationMatrix.getRowVectorRef(3).setD(1.0);
                break;
            case SDLK_DOWN:
                m_translationMatrix =
                    m_translationMatrix * Matrix::createTranslationMatrix(0.0, (-1.0) * kTranslationIncrement, 0.0);
                // TODO fix ths
                // m_translationMatrix.getRowVectorRef(3).setD(1.0);
                break;
            default:
 ...
 */
void MapPanel::drawTileOutlines(const std::vector<std::pair<int, int>>& tiles, CapEngine::Colour colour)
{
    ...
    for (auto&& tile : tiles) {
        ...
        Rect dstRect = {0, 0, static_cast<int>(round(scaledTileSize)), static_cast<int>(round(scaledTileSize))};
...
void MapPanel::drawSelectedTileOutlines() { drawTileOutlines(m_selectedTiles, selectedColour); }
...
void MapPanel::drawMouseDrag()
{
    ...
    if (m_dragState == DRAGSTATE_SELECT && m_dragStart.first != -1 && m_dragStart.second != -1) {
        ...
        int y = -1;
        SDL_GetMouseState(&x, &y);
        Rect rect = {m_dragStart.first, m_dragStart.second, x - m_dragStart.first, y - m_dragStart.second};
...
bool MapPanel::isInMap(int x, int y) const
{
    ...
    return x > mapExtents.x && x < mapExtents.x + mapExtents.w - 1 && y > mapExtents.y &&
           y < mapExtents.y + mapExtents.h - 1;
}
...
SDL_Rect MapPanel::getMapExtents() const
{
    ...
    return {static_cast<int>(mapOrigin.getX()), static_cast<int>(mapOrigin.getY()), static_cast<int>(mapDims.getX()),
...
SDL_Rect MapPanel::getVisibleMapExtents() const
{
    ...
    if (maybeVisibleMapExtents != std::nullopt) return *maybeVisibleMapExtents;
...
std::vector<std::tuple<int, int, bool, SDL_Rect>> MapPanel::getTileRects() const
{
    ...
    for (int x = 0; x < numTilesWide; x++) {
        for (int y = 0; y < numTilesHigh; y++) {
            ...
            Vector location(mapOrigin.getX() + (x * scaledTileSize), mapOrigin.getY() + (y * scaledTileSize), 0, 1);
