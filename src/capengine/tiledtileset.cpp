...
TiledTileset::TiledTileset(const jsoncons::json &in_data, int in_firstGid,
    ...
    : m_texture(getNullTexturePtr()), m_path(in_path), m_firstGid(in_firstGid)
{
    if (m_path)
    {
...
void TiledTileset::drawTileToTexture(SDL_Renderer *in_renderer,
                                     SDL_Texture *in_texture,
                                     unsigned int in_globalTileId) const
{
    CAP_THROW_NULL(in_renderer, "in_renderer is null.");
    CAP_THROW_NULL(in_renderer, "in_texture is null.");

    const auto tileID = in_globalTileId - m_firstGid;
}
}  // namespace CapEngine
