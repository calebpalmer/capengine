...
SDL_Rect TextBox::getPosition() const { return m_rect; }
...
void TextBox::update(double ms)
{
    ...
    if (m_cursorTimerMs + ms >= kCursorBlinkTime)
    {
        ...
        m_cursorTimerMs =
            (m_cursorTimerMs + static_cast<unsigned int>(ms + 0.5)) %
            kCursorBlinkTime;
    }
    else
    {
...
void TextBox::render()
{
    if (m_textureDirty) this->updateTexture();

    // draw the box
    assert(Locator::videoManager != nullptr);
    Locator::videoManager->drawFillRect(m_windowId, m_boxRect,
                                        m_displaySettings.backgroundColour);

    int textureWidth =
        m_texture == nullptr
            ? 0
            : Locator::videoManager->getTextureWidth(m_texture.get());

    if (m_textRect.x == 0 && m_textRect.y == 0 && m_textRect.w == 0 &&
        m_textRect.h == 0)
    {
    ...
    {
        Locator::videoManager->drawLine(
            m_windowId, m_textRect.x + cursorDrawOffset,
            m_boxRect.y + verticalPadding, m_textRect.x + cursorDrawOffset,
            m_boxRect.y + m_boxRect.h - verticalPadding, {0, 0, 0, 255});
    }
    ...
    {
        Defer defer(
            [&]() { Locator::videoManager->setClipRect(m_windowId, nullptr); });
        ...
        Locator::videoManager->drawTexture(m_windowId, m_textRect.x,
                                           m_textRect.y, m_texture.get());
    }
}

//! \copydoc Widget::canFocus
bool TextBox::canFocus() const { return true; }
...
void TextBox::updateTexture()
{
    ...
    if (m_text.size() > 0)
    {
        ...
        else
        {
            ...
            {
                beforeText = m_font.getTextSurface(
                    this->getTextBeforeSelection(), textColour);
            }

            // render the selected text
            Colour selectedTextColour{255, 255, 255, 255};
            Colour bgColour{100, 100, 100, 255};
            selectedText = m_font.getTextSurface(this->getSelectedText(),
            ...
            {
                afterText = m_font.getTextSurface(this->getTextAfterSelection(),
                                                  textColour);
            }
            ...
            int textureWidth =
                ...
                (beforeText != nullptr
                     ? Locator::videoManager->getSurfaceWidth(beforeText.get())
                     : 0) +
                (afterText != nullptr
            ...
            SurfacePtr combinedText = Locator::videoManager->createSurfacePtr(
            ...
            if (afterText != nullptr)
            {
                int xStart = beforeText != nullptr
                                 ? Locator::videoManager->getSurfaceWidth(
                                       beforeText.get()) +
                                       Locator::videoManager->getSurfaceWidth(
                                           selectedText.get())
                                 : Locator::videoManager->getSurfaceWidth(
                ...
                Locator::videoManager->blitSurface(
                    ...
                    combinedText.get(), xStart, 0);
            }

            m_texture = Locator::videoManager->createTextureFromSurfacePtr(
                m_windowId, combinedText.get());
        }
    }
    else
    {
...
TextBox::DisplaySettings TextBox::getDisplaySettings()
{
    ...
    const std::string fontColourPath =
        (boost::format("$.%1%.%2%.%3%") % kTextboxProperty %
         kFontColourProperty % kColourProperty)
            .str();

    boost::optional<jsoncons::json> maybeFontColourJson =
        configManager.getSetting(fontColourPath);

    if (maybeFontColourJson != boost::none)
    {
        boost::optional<Colour> maybeColour = getColour(*maybeFontColourJson);

        if (maybeColour != boost::none) settings.fontColour = *maybeColour;
    }

    // get the font size
    boost::optional<jsoncons::json> maybeFontSizeJson =
        configManager.getSetting(kDefaultFontSizeSettingsPath);
    ...
    boost::optional<jsoncons::json> maybeFont =
        configManager.getSetting(kDefaultFontSettingsPath);
    ...
    const std::string backgroundColourPath =
        (boost::format("$.%1%.%2%.%3%") % kTextboxProperty %
         kBackgroundColourProperty % kColourProperty)
            .str();
    ...
    {
        boost::optional<Colour> maybeColour =
            getColour(*maybeBackgroundColourJson);
...
void TextBox::deleteText()
{
    if (!this->isTextSelected())
    {
        ...
        {
            m_commandManager.runCommand(
                this->makeDeleteTextCommand(m_cursorPosition, 1));
        }
    }
    else
    {
...
bool TextBox::deleteSelectedText()
{
    if (this->isTextSelected())
    {
        auto deleteCommand = this->makeDeleteTextCommand(
            m_cursorSelectStart, m_cursorSelectEnd - m_cursorSelectStart);
...
std::unique_ptr<Command> TextBox::makeInsertTextCommand(std::string in_text)
{
    ...
    auto unExecuteCommand = [this,
                             currentCursorPosition = this->m_cursorPosition,
...
/**
 ...
 \param
  ...
  \return
                  \li The Command
*/
std::unique_ptr<Command> TextBox::makeDeleteTextCommand(std::size_t pos,
                                                        std::size_t len)
{
...
std::string TextBox::getSelectedText() const
{
    if (m_cursorSelectStart != m_cursorSelectEnd)
    {
        ...
        return m_text.substr(m_cursorSelectStart,
...
{
    m_keyPressHandlers.emplace(
        SDLK_BACKSPACE,
        std::bind(&TextBox::handleBackspaceKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_ESCAPE,
        std::bind(&TextBox::handleEscapeKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_DELETE,
        std::bind(&TextBox::handleDeleteKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_c, std::bind(&TextBox::handleCKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_x, std::bind(&TextBox::handleXKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_v, std::bind(&TextBox::handleVKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_z, std::bind(&TextBox::handleZKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_y, std::bind(&TextBox::handleYKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_a, std::bind(&TextBox::handleAKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_RIGHT,
        std::bind(&TextBox::handleRightArrowKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_LEFT,
        std::bind(&TextBox::handleLeftArrowKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(
        SDLK_HOME,
        std::bind(&TextBox::handleHomeKey, this, std::placeholders::_1));
    m_keyPressHandlers.emplace(SDLK_END, std::bind(&TextBox::handleEndKey, this,
                                                   std::placeholders::_1));
}
...
void TextBox::handleBackspaceKey(const SDL_KeyboardEvent &event)
{
    if (event.type == SDL_KEYDOWN)
    {
        ...
        else
        {
            ...
            {
                m_commandManager.runCommand(
                    this->makeDeleteTextCommand(m_cursorPosition - 1, 1));
            }
        }
    }
}
