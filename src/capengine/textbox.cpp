...
void TextBox::update(double ms)
{
  ...
  if (m_cursorTimerMs + ms >= kCursorBlinkTime) {
    ...
    m_cursorTimerMs = (m_cursorTimerMs + static_cast<unsigned int>(ms + 0.5)) %
                      kCursorBlinkTime;
  } else {
    m_cursorTimerMs += ms;
  }
...
void TextBox::render()
{
  ...
  if (m_texture != nullptr) {
    ...
    Locator::videoManager->drawTexture(m_windowId, m_textRect.x, m_textRect.y,
                                       m_texture.get());
  }
}
  ...
  if (m_text.size() > 0) {
    ...
    // text is selected
      ...
      if (m_cursorSelectStart > 0) {
        beforeText =
            m_font.getTextSurface(this->getTextBeforeSelection(), textColour);
      }
      ...
      if (m_cursorSelectEnd < static_cast<int>(m_text.size())) {
        afterText =
            m_font.getTextSurface(this->getTextAfterSelection(), textColour);
      }
      ...
      SurfacePtr combinedText =
          Locator::videoManager->createSurfacePtr(textureWidth, textureHeight);
...
TextBox::DisplaySettings TextBox::getDisplaySettings()
{
  ...
  const std::string fontColourPath =
      (boost::format("$.%1%.%2%.%3%") % kTextboxProperty % kFontColourProperty %
       kColourProperty)
          .str();
...
std::unique_ptr<Command> TextBox::makeInsertTextCommand(std::string in_text)
{
  ...
  auto unExecuteCommand = [this, currentCursorPosition = this->m_cursorPosition,
...
std::string TextBox::getTextAfterSelection() const
{
  if (m_cursorSelectStart != m_cursorSelectEnd &&
    ...
    return m_text.substr(m_cursorSelectEnd,
...
void TextBox::registerKeypressHandlers()
{
  ...
  m_keyPressHandlers.emplace(SDLK_LEFT, std::bind(&TextBox::handleLeftArrowKey,
                                                  this, std::placeholders::_1));
  m_keyPressHandlers.emplace(SDLK_HOME, std::bind(&TextBox::handleHomeKey, this,
                                                  std::placeholders::_1));
  m_keyPressHandlers.emplace(
      SDLK_END, std::bind(&TextBox::handleEndKey, this, std::placeholders::_1));
}
