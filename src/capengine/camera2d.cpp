                      ...
                      uint32_t in_windowId, bool doYFlip)
{
        ...
        Vector ret{in_point.x - static_cast<real>(viewingRect.x),
                   in_point.y - static_cast<real>(viewingRect.y)};

        if (doYFlip) {
            ret.y = (static_cast<real>(windowHeight) - ret.y);
        }
...
Vector windowToLocalCoordinate(Camera2d const& in_camera, Vector const& in_point, Rectangle const& in_rect, uint32_t windowId, bool doYFlip) {
	...
	return Vector{static_cast<real>(x), static_cast<real>(y)};
}
