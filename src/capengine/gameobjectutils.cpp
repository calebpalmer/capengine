...
*/
ObjectCreationError::ObjectCreationError(const std::string &in_details,
...
GameObject makeObject(const jsoncons::json &in_json)
{
    ...
    // metadata
    if (in_json.contains(Schema::Scene2d::kMetadata)) {
        for (auto &&property :
             in_json[Schema::Scene2d::kMetadata].object_range()) {
            auto &&propertyValue = property.value();

            object.setMetadata(std::string(property.key()),
                               JSONUtils::readMetadata(propertyValue));
        }
    }
    ...
    for (auto &&componentJson : in_json[kComponents].array_range()) {
        std::unique_ptr<Component> pComponent =
            componentFactory.makeComponent(componentJson);
