...
class VideoManager final
{
   public:
	...
	void setWindowLogicalResolution(uint32_t in_windowID, int in_width,
									int in_height);
	...
	std::shared_ptr<Texture> loadSharedImage(
		std::string const &in_filePath) const;
	void closeTexture(Texture *texture) const;
	void drawTexture(Uint32 windowID, Rect dstRect, Texture *texture,
					 Rect *srcRect = nullptr, bool applyTransform = true);
	void drawTexture(Uint32 windowID, Texture *texture, Rect *srcRect,
					 Rect *dstRect,
	...
	Texture *createTextureFromSurface(Uint32 windowID, Surface *surface,
									  bool freeSurface = false);
	TexturePtr createTextureFromSurfacePtr(Surface *surface,
										   bool freeSurface = false);
	TexturePtr createTextureFromSurfacePtr(Uint32 windowId, Surface *surface,
										   bool freeSurface = false);
	...
	void displayFPS(bool on, const std::string &ttfFontPath = "", Uint8 r = 0,
					Uint8 g = 0, Uint8 b = 0);

	// input
	void loadControllerMapFromFile(std::string filePath);
	// Drawing
	void drawLine(Uint32 windowID, int x1, int y1, int x2, int y2,
