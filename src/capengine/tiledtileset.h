...
class TiledTileset final
{
   public:
    explicit TiledTileset(
        std::string in_name, const jsoncons::json &in_data, int in_firstGid,
        std::optional<std::filesystem::path> in_path = std::nullopt);

    static TiledTileset create(std::string in_name,
                               std::filesystem::path in_path, int in_firstGid);
