 ...
 */
void AbsoluteLayout::addWidget(std::shared_ptr<Widget> pWidget, int x, int y,
                               int width, int height, Unit widthUnit,
                               Unit heightUnit)
{
  CAP_THROW_ASSERT(pWidget != nullptr, "Can't add null widget");
  CAP_THROW_ASSERT(x >= 0 && y >= 0, "Invalid X and Y");

  WidgetLocation widgetLocation = {pWidget, x,         y,         width,
                                   height,  widthUnit, heightUnit};
  ...
  // 	int newHeight = widgetLocation.y + widgetLocation.height <= m_y +
  // m_height ? 		height : m_height - y;
...
void AbsoluteLayout::updateWidgetPositions()
{
  ...
  for (auto &&widgetLocation : m_widgets) {
    ...
    SDL_Rect srcRect = {widgetLocation.x, widgetLocation.y,
                        widgetLocation.width, widgetLocation.height};
...
std::vector<std::shared_ptr<Widget>> AbsoluteLayout::getChildren()
{
  ...
  std::transform(m_widgets.begin(), m_widgets.end(), widgets.begin(),
                 [&widgets](const WidgetLocation &widgetLocation) {
                   return widgetLocation.pWidget;
                 });
