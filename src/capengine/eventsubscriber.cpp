...
EventSubscriber::EventSubscriber(
    std::optional<EventDispatcher *> eventDispatcher)
{
    if (eventDispatcher != std::nullopt) {
        int subscriptionMask = mouseEvent | keyboardEvent | systemEvent |
                               windowEvent | controllerEvent;
