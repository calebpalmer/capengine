...
//! default Constructor
WidgetState::WidgetState() : WidgetState([](WidgetState&) { return true; }, [](WidgetState&) { return true; }) {}
    ...
    : m_onLoadFunctor(onLoadFunctor), m_onDestroyFunctor(onDestroyFunctor)
{
    ...
    m_keyboardEventConnection = Locator::eventSubscriber->m_keyboardEventSignal.connect(
        std::bind(&WidgetState::handleKeyboardEvent, this, std::placeholders::_1));

    m_mouseButtonEventConnection = Locator::eventSubscriber->m_mouseButtonEventSignal.connect(
        std::bind(&WidgetState::handleMouseButtonEvent, this, std::placeholders::_1));

    m_mouseMotionEventConnection = Locator::eventSubscriber->m_mouseMotionEventSignal.connect(
        std::bind(&WidgetState::handleMouseMotionEvent, this, std::placeholders::_1));

    m_mouseWheelEventConnection = Locator::eventSubscriber->m_mouseWheelEventSignal.connect(
        std::bind(&WidgetState::handleMouseWheelEvent, this, std::placeholders::_1));

    m_windowEventConnection = Locator::eventSubscriber->m_windowEventSignal.connect(
        std::bind(&WidgetState::handleWindowEvent, this, std::placeholders::_1));

    m_textInputEventConnection = Locator::eventSubscriber->m_textInputEventSignal.connect(
        std::bind(&WidgetState::handleTextInputEvent, this, std::placeholders::_1));
...
std::shared_ptr<WindowWidget> WidgetState::createWindow(const std::string& name, int width, int height, bool resizable)
{
    ...
    m_windowConnections.push_back(pWindowWidget->registerWindowClosedSignal(
        std::bind(&WidgetState::handleWindowCloseSignal, this, std::placeholders::_1)));
    ...
    try {
        if (event.type == SDL_MOUSEBUTTONDOWN) {
            ...
            if (event.windowID == m_lastMouseDownWindowId) {
                auto maybeWindow =
                    std::find_if(m_windows.begin(), m_windows.end(), [this](std::shared_ptr<WindowWidget> pWindow) {
...
void WidgetState::handleWindowCloseSignal(WindowWidget* pWindowWidget)
{
    ...
    if (window != m_windows.end()) m_windows.erase(window);
        ...
        if (widget->canFocus()) {
            ...
            if (handled) {
                if (m_pFocusedWidget != nullptr && m_pFocusedWidget.get() != widget.get()) {
                    m_pFocusedWidget->doFocus(false, m_lastMouseDownPosition.first, m_lastMouseDownPosition.second, x,
                                              y);
                }
