...
*/
void checkForFinishedControls(
    std::vector<std::shared_ptr<UI::Control>> &controls)
{
...
WidgetState::WidgetState()
    : WidgetState([](WidgetState &) { return true; },
                  [](WidgetState &) { return true; })
{
}
...
/**
                ...
                \param onDestroyFunctor - Function to call when destroying the
   GameState
*/
WidgetState::WidgetState(
    std::function<bool(WidgetState &widgetState)> onLoadFunctor,
    ...
    : m_onLoadFunctor(onLoadFunctor), m_onDestroyFunctor(onDestroyFunctor)
{
  ...
  m_keyboardEventConnection =
      Locator::eventSubscriber->m_keyboardEventSignal.connect(std::bind(
          &WidgetState::handleKeyboardEvent, this, std::placeholders::_1));

  m_mouseButtonEventConnection =
      Locator::eventSubscriber->m_mouseButtonEventSignal.connect(std::bind(
          &WidgetState::handleMouseButtonEvent, this, std::placeholders::_1));

  m_mouseMotionEventConnection =
      Locator::eventSubscriber->m_mouseMotionEventSignal.connect(std::bind(
          &WidgetState::handleMouseMotionEvent, this, std::placeholders::_1));

  m_mouseWheelEventConnection =
      Locator::eventSubscriber->m_mouseWheelEventSignal.connect(std::bind(
          &WidgetState::handleMouseWheelEvent, this, std::placeholders::_1));

  m_windowEventConnection =
      Locator::eventSubscriber->m_windowEventSignal.connect(std::bind(
          &WidgetState::handleWindowEvent, this, std::placeholders::_1));

  m_textInputEventConnection =
      Locator::eventSubscriber->m_textInputEventSignal.connect(std::bind(
          &WidgetState::handleTextInputEvent, this, std::placeholders::_1));
...
/**
                \param onLoadFunctor - Code to call when loading the GameState
                \param onDestroyFunctor - Function to call when destroying the
   GameState \return - The WidgetState
*/
std::shared_ptr<WidgetState> WidgetState::create(
    std::function<bool(WidgetState &widgetState)> onLoadFunctor,
    std::function<bool(WidgetState &widgetState)> onDestroyFunctor)
{
  std::shared_ptr<WidgetState> pWidgetState(
      new WidgetState(onLoadFunctor, onDestroyFunctor));
...
bool WidgetState::onLoad() { return m_onLoadFunctor(*this); }

//! @copydoc GameState::onDestroy()
bool WidgetState::onDestroy() { return m_onDestroyFunctor(*this); }
...
*/
std::shared_ptr<WindowWidget> WidgetState::createWindow(const std::string &name,
                                                        int width, int height,
                                                        bool resizable)
{
  // not using CapEngine::createWindow because we want to handle receiving the
  // event signals here and passing them to any child windows
  std::shared_ptr<WindowWidget> pWindowWidget =
      WindowWidget::create(name, width, height, resizable);
  ...
  m_windowConnections.push_back(
      pWindowWidget->registerWindowClosedSignal(std::bind(
          &WidgetState::handleWindowCloseSignal, this, std::placeholders::_1)));
...
*/
boost::signals2::scoped_connection WidgetState::connectPostRenderSignal(
    std::function<void(WidgetState &)> slot)
{
...
*/
void WidgetState::showOkCancelDialog(const std::string &msg,
                                     std::function<void(bool)> callback)
{
  std::shared_ptr<DialogState> pDialogState =
      DialogState::create(msg, callback);
...
void WidgetState::handleWindowCloseSignal(WindowWidget *pWindowWidget)
{
  auto window =
      std::find_if(m_windows.begin(), m_windows.end(),
                   [&](const decltype(m_windows)::value_type &pWindow) {
...
*/
bool WidgetState::handleMouseFocusChange(std::shared_ptr<Widget> widget, int x,
                                         int y)
{
  try {
    ...
    bool handled = false;
    if (widget->canFocus()) {
      handled = widget->doFocus(true, m_lastMouseDownPosition.first,
                                m_lastMouseDownPosition.second, x, y);
      if (handled) {
        if (m_pFocusedWidget != nullptr &&
            m_pFocusedWidget.get() != widget.get()) {
          m_pFocusedWidget->doFocus(false, m_lastMouseDownPosition.first,
                                    m_lastMouseDownPosition.second, x, y);
        }
