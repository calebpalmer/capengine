...
TileSet::TileSet(const string& configPath)
{
    ...
    jsoncons::json_stream_reader reader(configIn, decoder);
...
TileSet::TileSet(const jsoncons::json& json)
{
    ...
    for (auto&& jsonTile : json[kTileArrayProperty].array_range()) {
        try {
            ...
            BOOST_LOG_SEV(CapEngine::log, boost::log::trivial::error) << e.what();
        }
    }
}
...
Tile TileSet::parseTile(const string& line)
{
    ...
    if (position == string::npos) {
        stringstream errorMessage;
        errorMessage << "Unable to parse tile in config file "
                     << m_configFilePath;
        BOOST_THROW_EXCEPTION(CapEngineException(errorMessage.str()));
    }
...
void TileSet::validate()
{
    if (m_tileCount != m_tiles.size()) {
        BOOST_THROW_EXCEPTION(CapEngineException(
            "Tiles read does not equal declared tile count"));
    }
...
jsoncons::json TileSet::json() const
{
    ...
    for (size_t i = 0; i < m_tiles.size(); i++) {
        ...
        jsonTile.insert_or_assign(kTileTypeProperty,
                                  tileTypeToString(tile.type));
