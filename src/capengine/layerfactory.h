...
struct LayerCreationException : public CapEngineException {
  explicit LayerCreationException(const std::string &in_type,
                                  const jsoncons::json &in_json,
                                  const std::string &in_details)
      : CapEngineException([&]() {
          std::stringstream msg;
          msg << "Layer of type \"" << in_type << "\" could not be created."
              << std::endl
...
public:
  using factoryfunc_t =
      std::function<std::unique_ptr<Layer>(const jsoncons::json &)>;
...
public:
  ...
  void registerLayerType(const std::string &in_type,
