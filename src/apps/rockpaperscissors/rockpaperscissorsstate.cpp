...
CapEngine::Rect getTilePosition(CapEngine::TiledTileset const& in_tileset,
                                RPS::Choice in_choice)
{
    switch (in_choice) {
    ...
    : m_windowId(in_windowId)
{
    ...
    std::filesystem::path tiledMapPath =
        std::filesystem::path{
            std::filesystem::path{CapEngine::getCurrentExecutablePath()}
                .parent_path()
                .parent_path()} /
    ...
    m_state.settings.player1Keybindings =
        PlayerKeybindings{SDLK_a, SDLK_s, SDLK_d};
    m_state.settings.player2Keybindings =
        PlayerKeybindings{SDLK_LEFT, SDLK_DOWN, SDLK_RIGHT};

    // register for keyboard events
    CapEngine::Locator::getEventSubscriber().m_keyboardEventSignal.connect(
        [this](SDL_KeyboardEvent in_event) {
            this->handleKeyboardEvent(in_event);
        });
}
...
void RockPaperScissorsState::renderPlayers()
{
    ...
    assert(objectGroups.size() == 1);
    auto const& objectGroup = objectGroups[0];

    videoManager.drawTexture(m_windowId, m_state.player1.drawPosition, *texture,
                             &m_state.player1.tilePosition);
    videoManager.drawTexture(
        m_windowId, *texture, &m_state.player2.tilePosition,
...
void RockPaperScissorsState::update(double ms)
{
    ...
    if (m_state.phase == GamePhase::Start ||
...
void RockPaperScissorsState::updateStopped(double in_ms)
{
    ...
    m_state.player1.drawPosition = {static_cast<int>(maybeLeftObject->x),
                                    static_cast<int>(maybeLeftObject->y),
                                    (*tileset)->tileWidth(),
                                    (*tileset)->tileHeight()};
    ...
    m_state.player2.drawPosition =
        CapEngine::Rect{static_cast<int>(maybeRightObject->x),
                        static_cast<int>(maybeRightObject->y),
...
void RockPaperScissorsState::updateChoose(double in_ms)
{
    ...
    if (m_state.choiceTimeMs < kChoiceTimeMs) {
        ...
        double velocity =
            (leftObjectEnd->y - leftObjectStart->y) / timePerMovementMs;
    ...
    // choice time is over but wait a bit to show the end choices
        ...
        if(m_state.phase == GamePhase::Choice){
            ...
            m_state.player1.tilePosition =
                getTilePosition(**tileset, m_state.player1.currentChoice);
            m_state.player2.tilePosition =
                getTilePosition(**tileset, m_state.player2.currentChoice);
            ...
            else if(m_state.player1.currentChoice == Choice::Paper && m_state.player2.currentChoice == Choice::Scissors){
                m_state.player2.score += 1;
            }
            else if(m_state.player1.currentChoice == Choice::Scissors && m_state.player2.currentChoice == Choice::Paper){
                m_state.player1.score += 1;
            }
            else if(m_state.player1.currentChoice == Choice::Scissors && m_state.player2.currentChoice == Choice::Rock){
                m_state.player2.score += 1;
            }
...
void RockPaperScissorsState::handleKeyboardEvent(SDL_KeyboardEvent in_event)
{
    if (m_state.phase == GamePhase::Start ||
        ...
        if (in_event.type == SDL_KEYUP &&
            in_event.keysym.sym ==
                m_state.settings.globalKeybindings.startKeycode) {
        ...
        if (in_event.type == SDL_KEYUP) {
            if (in_event.keysym.sym ==
                    m_state.settings.player1Keybindings.rockKeycode &&
                m_state.player1.nextChoice == std::nullopt) {
                m_state.player1.nextChoice = Choice::Rock;
            } else if (in_event.keysym.sym ==
                           m_state.settings.player1Keybindings.paperKeycode &&
                       m_state.player1.nextChoice == std::nullopt) {
                ...
                m_state.player1.nextChoice = Choice::Scissors;
                           ...
                           m_state.settings.player2Keybindings.rockKeycode &&
                       m_state.player2.nextChoice == std::nullopt) {
                m_state.player2.nextChoice = Choice::Rock;
            } else if (in_event.keysym.sym ==
                           m_state.settings.player2Keybindings.paperKeycode &&
                       m_state.player2.nextChoice == std::nullopt) {
