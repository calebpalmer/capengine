...
*/
BitmapCollisionLayer::BitmapCollisionLayer(int in_assetId, Rectangle in_position)
	: ImageLayer(in_assetId, in_position)
{
}

//! \copydoc Layer::checkCollision
BitmapCollisionLayer::CollisionType BitmapCollisionLayer::checkCollision(const GameObject &in_object){
	assert(Locator::assetManager != nullptr);

	SoftwareImage softwareImage = Locator::assetManager->getSoftwareImage(m_assetId);
	assert(softwareImage.surface != nullptr);

	Vector temp;
	CapEngine::CollisionType collisionType = detectBitmapCollision(in_object.boundingPolygon(), softwareImage.surface, temp);
