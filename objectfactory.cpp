...
std::function<std::shared_ptr<GameObject>()> ObjectFactory::getFactoryFunction(const std::string& objectType){
...
void ObjectFactory::registerFactoryFunction(const std::string &objectType,
				   std::function<std::shared_ptr<GameObject> ()> inFunction)
{
  m_functions[objectType] = inFunction;
}

std::shared_ptr<GameObject> ObjectFactory::makeObject(const std::string& objectType){
