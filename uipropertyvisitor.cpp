...
void UIPropertyVisitor::visitList(Property &property){
	std::optional<ListProperty> maybeListProperty = property.getValueAs<ListProperty>();
	assert(maybeListProperty != std::nullopt);

	std::shared_ptr<LinearLayout> pLayout = LinearLayout::create(LinearLayout::Orientation::Vertical);
	assert(pLayout != nullptr);
