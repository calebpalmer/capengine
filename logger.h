...
class Logger
{
  ...
  public:
    ...
    void log(const std::string &message, LogLevel level = CWARNING,
             char const *file = "", int lineno = -1);
    void log(const std::exception &e, LogLevel level = CWARNING,
             char const *file = "", int lineno = -1);
    void log(const jsoncons::json_exception &e, LogLevel = CWARNING,
             char const *file = "", int lineno = -1);
